...
class ExoplanetClassifier:
    def __init__(self, contamination=0.08, rf_trees=800):
        ...
        self.anomaly_detector = IsolationForest(
            contamination=contamination, random_state=None, n_jobs=-1
        )
        ...
        self.best_threshold = 0.5  # tuned later
    ...
    def preprocess(self, df, fit=True):
        ...
        return Z, y, hz.reset_index(drop=True)
        ...
        for i in range(iters):
            ...
            for name, base in self.models.items():
                ...
                if hasattr(m, "predict_proba"):
                    ...
                    p = m.predict(Xte)  # HGB has predict_proba; fallback just in case
    ...
    def train(self, X, y, use_tuning=True, mc_iters=60, use_smote=False, calibrate=False, ensemble=False):
        ...
        # 3) holdout for threshold + calibration
        Xtr, Xva, ytr, yva = train_test_split(Xf, yf, test_size=0.2, stratify=yf, random_state=None)
        self.fit_final(Xtr, ytr, use_calibration=calibrate)
    ...
    def evaluate(self, X, y, hz_flag_test: pd.Series):
        ...
        true_planets = (yf == 1)
        n_true = int(true_planets.sum())
        n_true_hz = int((true_planets & (hz_f == 1)).sum())

        pred_planets = (preds == 1)
        n_pred = int(pred_planets.sum())
        n_pred_hz = int((pred_planets & (hz_f == 1)).sum())
        ...
        print(f"True CONFIRMED planets: {n_true}")
        print(f"  In HZ (true): {n_true_hz}  -> {pr_true:.2f}%")
        print(f"Predicted planets:      {n_pred}")
        print(f"  In HZ (pred):  {n_pred_hz}  -> {pr_pred:.2f}%")
...
def main():
    ...
    clf = ExoplanetClassifier(contamination=args.contamination, rf_trees=args.rf_trees)

    df = clf.load(args.data)
    df = clf.targetize(df)

    # Global HZ ratio among true confirmed (informative only)
    ...
    X, y, hz = clf.preprocess(df, fit=True)

    Xtr, Xte, ytr, yte, hz_tr, hz_te = train_test_split(
        X, y, hz, test_size=args.test_size, stratify=y, random_state=None
    )
    ...
    stats = clf.evaluate(Xte, yte, hz_te)
